...
def force_sandbox() -> bool:
    """Distribution lock: when ``CHARA_FORCE_SANDBOX`` is set at startup, EVERY chara
    is pinned to the sandbox jail and ``admin`` is refused — so a hosted/shared OpenCharaAgent
    ...
    return os.environ.get("CHARA_FORCE_SANDBOX", "").strip().lower() in {"1", "true", "yes", "on"}


def backend() -> str:
    """Isolation mechanism for this session (CHARA_PY_BACKEND: sandbox|admin).
...
def _chara_home() -> Path:
    ...
    return Path(os.environ.get("CHARA_HOME", str(Path.home() / ".chara"))).expanduser()
...
def _macos_profile(workspace: Path, allow_network: bool, writable: list[Path], *,
    ...
    if browser:
        ...
        # opening an ABSOLUTE path there (file:///…/.chara/…/workspace/x.html —
        # the chara browsing its own generated files) requires stat/lookup on each
        # ancestor (.chara, sessions, <name>, sandbox). The shell escapes this
...
{b_net}'''
    ...
    # AND the OpenCharaAgent home (the global key in desktop.json, the login hash in
    # auth.json, every OTHER chara's session), then re-allow ONLY this chara's
    # own workspace + assets. Previously only ~/.chara was denied, so a chara's
...
def _linux_jail_argv(inner: list[str], workspace: Path, allow_network: bool, writable: list[Path],
    ...
    if browser:
        ...
        cmd += ["--tmpfs", str(_chara_home())]          # hide global key + other sessions
...
def _linux_landlock_argv(inner: list[str], workspace: Path, allow_network: bool, writable: list[Path],
    ...
    ``python -m chara.session.landlock`` so the restriction is inherited.
    ...
    """
    ws = str(workspace)
    args = [sys.executable, "-m", "chara.session.landlock"]
    for ro in ("/usr", "/lib", "/lib64", "/bin", "/sbin", "/etc", sys.prefix):
        args += ["--ro", ro]
    if browser:
        ...
        # roots) + shared rw /tmp (socket + Chrome profile). ~/.chara is simply
...
def build_jail_command(
    ...
    *,
    ...
    (a chara could read the global key in ~/.chara). NEVER degrades to
    ...
    if isolation == "sandbox":
        ...
        if landlock_available():
            ...
            if not browser:
                ...
                note += (
                    "\n[chara: Landlock jail — /proc is unavailable by policy, so "
            ...
            if not allow_network:
                ...
                note += "\n[chara: Landlock jail — filesystem confined, but network not gated (ABI v1)]"
                ...
                if not _LANDLOCK_NETOFF_WARNED:
                    ...
                    logging.getLogger("chara.isolation").warning(
...
def interactive_shell_notice(isolation: str, *, allow_network: bool = False) -> str:
    ...
    return "[chara: Landlock jail — " + "; ".join(parts) + "]"
