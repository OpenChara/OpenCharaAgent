...
``CharaChild`` owns a long-lived ``chara serve <name> --stdio`` subprocess:
...
def _driver_send_timeout() -> float:
    ...
    """
    pkg = sys.modules.get("chara.server.supervisor")
    if pkg is not None:
...
class CharaChild:
    ...
    SUSPEND_AFTER = float(os.environ.get("CHARA_SUSPEND_AFTER", "1800"))
