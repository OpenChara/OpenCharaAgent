...
_log = logging.getLogger("chara.server.hub")
...
def _foreign_to_card(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    ext_in = se.get("chara") if isinstance(se, dict) and isinstance(se.get("chara"), dict) else {}
...
def _sanitize_card_extensions(card: dict[str, Any]) -> None:
    ...
    if not isinstance(chara, dict):
        return
    svg, _note = _sanitize_avatar_svg(chara.get("avatar_svg"))
    if svg:
        chara["avatar_svg"] = svg
    else:
        chara.pop("avatar_svg", None)
    # avatar_file (sidecar reference): keep only a bare, traversal-free filename.
    af = chara.get("avatar_file")
    if isinstance(af, str) and af.strip() and "/" not in af and "\\" not in af and ".." not in af:
        chara["avatar_file"] = af.strip()
    else:
        chara.pop("avatar_file", None)
    # Dual theme {primary, secondary}; fold a legacy theme_color into primary.
    theme = _clean_theme(chara.get("theme"), chara.get("theme_color"))
    if theme:
        chara["theme"] = theme
    else:
        chara.pop("theme", None)
    chara.pop("theme_color", None)
    # The card FIELD is a boolean; bridge a legacy `embodiment: "actor"`, else omit.
    forced = normalize_force_roleplay(chara.get("force_roleplay"))
    if forced is None and str(chara.get("embodiment") or "").lower() == "actor":
        forced = True
    chara.pop("embodiment", None)
    if forced:
        chara["force_roleplay"] = True
    else:
        chara.pop("force_roleplay", None)


def _safe_extensions_for_ui(extensions: dict[str, Any]) -> dict[str, Any]:
    """Copy card extensions with chara visual fields sanitized for rendering."""
...
def _trash_cards_dir() -> Path:
    d = S.chara_home() / ".trash" / "cards"
    d.mkdir(parents=True, exist_ok=True)
    return d


def delete_card(path: str) -> dict[str, Any]:
    """SOFT delete: move the card file into ~/.chara/.trash/cards/<id>/ (with an
