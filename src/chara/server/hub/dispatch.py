...
def _brief_of(card: dict) -> dict | None:
    """The visual brief persisted on a card (extensions.chara.visual_brief), or None."""
    lm = (((card.get("data") or {}).get("extensions") or {}).get("chara") or {})
...
class HubDispatcher:
    ...
    def _hub_state(self, p: dict[str, Any]) -> Any:
        ...
        return {
            ...
            "home": str(S.chara_home()),
            # Distribution lock (CHARA_FORCE_SANDBOX): the UI greys the sandbox toggle.
    ...
    def _gateway_start(self, p: dict[str, Any]) -> Any:
        meta = _meta(p)
        if self.supervisor is None:
            raise RpcError(-32060, "gateway supervision requires charad")
    ...
    def _card_visual_brief(self, p: dict[str, Any]) -> Any:
        # The visual brief is PERSISTED on the card (extensions.chara.visual_brief)
