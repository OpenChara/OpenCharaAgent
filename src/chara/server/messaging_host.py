...
Unlike the standalone :class:`~chara.messaging.gateway.MessagingGateway`
...
_log = logging.getLogger("chara.server.messaging_host")
...
class MessagingHost:
    ...
    def _ensure_relay(self) -> None:
        ...
        self._relay = threading.Thread(
            target=self._relay_loop, name="chara-messaging-relay", daemon=True,
        )
        self._relay.start()
    ...
    def _start_one(self, adapter: "Adapter") -> None:
        ...
        th = threading.Thread(
            ...
            name=f"chara-{adapter.name}-adapter", daemon=True,
        )
        th.start()
