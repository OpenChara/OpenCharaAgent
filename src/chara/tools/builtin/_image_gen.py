...
(``image_provider`` / ``image_model`` in ``~/.chara/desktop.json``) — no
...
def _desktop_json() -> dict:
    """The global web keyring/defaults at ``~/.chara/desktop.json`` (honoring
    ``CHARA_HOME``). Read directly, stdlib-only — ``tools/`` must never import
