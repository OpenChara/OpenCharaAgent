...
onto OpenCharaAgent's one-process-one-chara runtime.
...
Divergences from hermes (documented honestly, never a fake success):
  ...
  * Tool dispatch: hermes re-enters ``model_tools.handle_function_call``; we call
    ...
    gateway — feasible exactly because OpenCharaAgent is one-process-one-chara).
...
logger = logging.getLogger("chara.tools.execute_code")
...
# ---------------------------------------------------------------------------
# chara_tools.py stub generator (hermes generate_hermes_tools_module, :211-291)
# ---------------------------------------------------------------------------
...
"""Auto-generated OpenCharaAgent tools RPC stubs (file-based transport)."""
...
def execute_code(args: dict, ctx) -> str:
    ...
    try:
        ...
        (stage_dir / "chara_tools.py").write_text(tools_src, encoding="utf-8")
        ...
        # stage dir so `from chara_tools import ...` resolves. We force
...
def build_execute_code_schema(enabled_sandbox_tools=None) -> dict:
    ...
    description = (
        ...
        f"Available via `from chara_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into chara_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from chara_tools import {import_str}` "
