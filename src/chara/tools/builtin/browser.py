...
+ browser_dialog_tool.py), re-implemented against OpenCharaAgent's runtime.
...
check; OpenCharaAgent has no CDP supervisor, so browser_cdp/browser_dialog here drive
...
def browser_snapshot(args: dict, ctx) -> str:
    ...
    # (hermes merges supervisor state; OpenCharaAgent reads it straight from the CLI).
...
def browser_vision(args: dict, ctx) -> str:
    ...
    # Under the WORKSPACE, not ~/.chara/cache: every browser jail confines
    ...
    # (an absolute ~/.chara path silently dropped the attachment).
    ...
    # when the active model lacks native vision; OpenCharaAgent has one provider client).
...
def browser_cdp(args: dict, ctx) -> str:
    ...
    WebSocket supervisor / the `websockets` package; OpenCharaAgent has no supervisor,
...
def browser_dialog(args: dict, ctx) -> str:
    ...
    CDPSupervisor; OpenCharaAgent drives agent-browser's `dialog` verb (accept/dismiss
