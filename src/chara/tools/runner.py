...
def run_terminal_result(
    ...
    *,
    ...
    clamp_note = (
        f"\n[chara: timeout clamped to {timeout}s (requested {requested}s; allowed {MIN_TIMEOUT}-{MAX_TIMEOUT}s)]"
    ...
    try:
        ...
        # whole container, incl. the global key in ~/.chara (OPEN-WORK SEC-low).
        return TerminalResult((f"[chara: refused — {e}]" + note).strip(), refused=True)
    note += jail_note
...
# OpenCharaAgent is stdlib-only (macOS/Linux only), so we allocate the pty the same way
...
def run_terminal_pty(
    ...
    *,
    ...
    try:
        ...
        return (f"[chara: refused — {e}]" + note).strip()
    note += jail_note
    run_cwd = str(cwd) if jail_cwd is not None else None
