...
home; OpenCharaAgent is one-process-one-chara, so storage stays PER-CHARA under the
...
# fcntl is Unix-only; on Windows fall back to msvcrt (OpenCharaAgent is mac/linux, but
...
class MemoryStore:
    ...
    OpenCharaAgent divergence: the storage directory is per-chara (passed in), not a
    ...
    def _memory_dir(self) -> Path:
        # Resolved per call (hermes resolves dynamically); for OpenCharaAgent it is the
    ...
    # ---- OpenCharaAgent public API the agent + chara-life tools rely on -------------
