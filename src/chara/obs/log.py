...
- Files live next to the audit trail: `<sandbox>/logs/chara.log` (everything)
  and `errors.log` (WARNING+). One chara = one process = one log dir.
- Every record carries the chara name (CHARA_SESSION) so logs copied off a
...
- NOTHING goes to stdout/stderr — the TUI owns the terminal. `CHARA_DEBUG=1`
...
def debug_enabled() -> bool:
    return os.getenv("CHARA_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"}
...
class _SessionAndRedact(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        record.session = os.getenv("CHARA_SESSION", "-")
        return True
...
def setup_logging(debug: "bool | None" = None, directory: "Path | None" = None, force: bool = False) -> Path:
    ...
    main = logging.handlers.RotatingFileHandler(
        target / "chara.log", maxBytes=5_000_000, backupCount=3, encoding="utf-8"
    )
...
def get_logger(name: str) -> logging.Logger:
    """Component logger: get_logger('llm') -> 'chara.llm'. Usable before
    setup_logging (records just go nowhere — propagate is only cut at setup)."""
    return logging.getLogger(f"chara.{name}")
