...
_log = logging.getLogger("chara.messaging.gateway")

DEFAULT_REFUSAL = (
    "Sorry, this OpenCharaAgent messaging gateway only accepts messages from its "
    "configured contacts."
)
...
def config_path() -> Path:
    root = os.getenv("CHARA_CONFIG_DIR")
    if root:
        return Path(root).expanduser().resolve() / "messaging.json"
    session = os.getenv("CHARA_SESSION", "")
    if session:
        return Path.home().expanduser() / ".chara" / "sessions" / session / "messaging.json"
    raise RuntimeError("no active OpenCharaAgent session; activate a session before loading messaging config")
    ...
    def start(self) -> None:
        ...
        for adapter in self.adapters:
            thread = threading.Thread(
                ...
                args=(adapter,),
                name=f"chara-{adapter.name}-adapter",
                daemon=True,
            )
            thread.start()
