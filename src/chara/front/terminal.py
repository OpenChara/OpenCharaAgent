...
from ..content.themes import CHARA_BANNER
...
BANNER = (
    CHARA_BANNER
    + "\nOPENCHARAAGENT // LOCAL AGENTIC CHARACTER RUNTIME"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='OpenCharaAgent plain terminal mode (legacy; the TUI is the default)')
    ...
    # sessions.pid_is_chara can tell sibling daemons apart after reboot pid-reuse.
    parser.add_argument('--session', default='', help=argparse.SUPPRESS)
    args = parser.parse_args(argv)
    if args.debug:
        import os

        os.environ['CHARA_DEBUG'] = '1'  # picked up by setup_logging in the agent
    ...
    # by `chara start`) differ only in whether THIS frontend drives input —
