...
class TuiTheme:
    ...
    name: str = "OpenCharaAgent"
    # --- decorative text ---
    banner: str = CHARA_BANNER
    subtitle: str = "OPENCHARAAGENT  ·  agentic character tavern"
    tagline: str = "RUNTIME READY."
    quit_line: str = "Session closed."
    display_title: str = "OPENCHARAAGENT // CHARACTER"
...
def load_theme(path: str | None) -> TuiTheme:
    """Load a theme by path; fall back to the built-in OpenCharaAgent default on any problem."""
    p = (path or "").strip()
    if not p:
        return TuiTheme()
    try:
