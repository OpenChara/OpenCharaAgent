...
def _home() -> Path:
    return Path(os.getenv("CHARA_HOME", str(Path.home() / ".chara")))
...
def _desktop_json() -> dict:
    """The global keyring/defaults at ``~/.chara/desktop.json`` (stdlib-only;
...
def selected_model() -> str:
    """The active matte model. Order: env ``CHARA_MATTE_MODEL`` → the global
    ...
    m = (os.getenv("CHARA_MATTE_MODEL") or "").strip()
    if m in MODELS:
        return m
...
def download(model_id: str, *, progress_cb=None, chunk: int = 1 << 20) -> Path:
    ...
    try:
        req = urllib.request.Request(spec.url, headers={"User-Agent": "chara"})
