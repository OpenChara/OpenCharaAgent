...
``tools/file_operations.py``, apple-to-apple behavior, re-anchored to LunaMoth).

hermes' ``ShellFileOperations`` shells every read/write/stat through a terminal
backend so it can run on remote/docker/ssh environments. LunaMoth runs one chara
...
class FileOps:
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        tmp = resolved.with_name(f".{resolved.name}.lunamoth-tmp")
        try:
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        # compiled-language shell linters are LunaMoth-optional; only the
