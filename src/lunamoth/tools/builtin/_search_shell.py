...
LunaMoth's ``ctx.run_terminal`` returns a FORMATTED string (``exit=N`` /
...
def _parse_runner_output(raw: str) -> tuple[str, str]:
    """Split LunaMoth runner output into (stdout_section, stderr_section).

    The runner formats success as ``exit=N\\nSTDOUT:\\n...\\nSTDERR:\\n...`` with
    optional ``[lunamoth: ...]`` notes appended. We merge STDOUT+STDERR into one
    ...
    out_section = ""
    err_section = ""
    # Drop runner notes (they begin with "[lunamoth:" / "[timed out" / "[runner")
    lines = raw.split("\n")
    mode = None
    out_lines: list[str] = []
    err_lines: list[str] = []
    for line in lines:
        ...
        if line.startswith("STDERR:"):
            mode = "err"
            continue
        if line.startswith("exit=") or line.startswith("[lunamoth:") \
