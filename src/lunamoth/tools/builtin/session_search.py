...
LunaMoth's per-chara transcript SQLite (core/transcript.py).

KEY ADAPTATION (per .codex-fleet/spec-general-tools.md §4 + seam):
  - LunaMoth has ONE chara per process and uses **epochs** (the /reset boundary),
  ...
  - The hermes multi-profile layer (profile=, @session: links, cross-profile DB
    scan) and parent/child lineage are STRIPPED — LunaMoth has no profile
...
logger = logging.getLogger("lunamoth.tools.session_search")
...
SESSION_SEARCH_SCHEMA = {
    "description": (
        ...
        "shape returns actual messages from the DB. (LunaMoth: a 'session' is a "
