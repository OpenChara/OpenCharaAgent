...
def _http_get(url: str, timeout: float = _DOWNLOAD_TIMEOUT) -> bytes:
    req = urllib.request.Request(
        url, headers={"Accept": "application/octet-stream", "User-Agent": "lunamoth"})
...
def download_verified_wheel(wheel_url: str, sums_url: str) -> tuple[Path, str]:
    ...
    fd, tmp = tempfile.mkstemp(prefix="lunamoth-", suffix=".whl")
    try:
...
def apply() -> dict[str, Any]:
    ...
    try:
        if is_dev():
            ...
            steps = [[uv, "tool", "install", "--force", f"lunamoth[{EXTRAS}] @ {target}"]]
        for cmd in steps:
